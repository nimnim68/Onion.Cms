...
namespace Onion.Cms.Web.Areas.Panel.Controllers
{
    ...
    public class ProfileController : BaseController<ApplicationUser>
    {
        ...
        public async Task<IActionResult> UserInfo()
        {
            ...
            var userId = Convert.ToInt16(claim?.Value);
            var model = new ApplicationUserInfoDto() { IsUpdated = false, ApplicationUserId = userId };
            var cmd = await Mediator.Send(new GetApplicationUserInfoQuery() { ApplicationUserId = userId });
            if (cmd != null)
            {
